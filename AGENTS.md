...
本仓库用于“学生托管机构智能化系统平台”的多租户 SaaS 规划与原型交付。当前唯一阶段是 **Phase 1A 批次A**；当前批次只产出治理、规格、架构草案、低保真原型和审查证据，不是正式业务开发。

禁止自动进入 Phase 1A 批次B、Phase 1B 或其他阶段。任何阶段结束后必须验证、提交证据并停止，等待项目负责人验收。
...
- A0—A7 一律禁止安装依赖；浏览器验证失败时必须改用静态链接与内容检查，不得为截图或浏览器自动化安装工具。
