...
本仓库用于“学生托管机构智能化系统平台”的多租户 SaaS 规划、原型与早期基础实现。Phase 1A Batch A、Batch B 均为 `HISTORICAL/FROZEN`；当前追认 Phase 1B Task 01—05 已完成并冻结，Task 06 Stage B 已实施且通过内部验证，C1 证据已生成，当前停在负责人审阅闸门。

当前活动治理以 `docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V5.md` 为准（SHA-256：`BC8B2232F3203368BD712586464734614D0E56D062792AFA284F8794A50914DB`）。Task 01—05 已由负责人验收并冻结；Task 06 Stage B 已获明确授权、实施并通过内部验证，Stage C1 已生成证据并等待负责人审阅；Stage C2 与 Task 07+ 仍未授权。任何新任务仍须独立合同、稳定 SHA、项目负责人批准和单独实现授权；不得从已完成任务自动推导后续任务授权。
...
Batch B 固定交付为 47 个 A 级页面、12 个 B 级流程、59 个浏览器路由、15 张真实浏览器截图、V0.1 技术基线、18 个独立 TDD 任务的 Phase 1B 计划、自动验证器及 86 项精确白名单审查包。上述 Batch B 合同与证据保持历史冻结。当前 Phase 1B 活动治理以 `docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V5.md` 为准（SHA-256：`BC8B2232F3203368BD712586464734614D0E56D062792AFA284F8794A50914DB`）：Task 05 C2 已完成并冻结，Task 06 Stage B 已实施并通过内部验证，Stage C1 已生成并等待负责人审阅，`PHASE_1B_STARTED=YES_FOR_TASK_01_TO_TASK_06_STAGE_B_ONLY`，`TASK_06_STARTED=YES_STAGE_B_IMPLEMENTATION`，`TASK_06_IMPLEMENTATION_AUTHORIZATION=GRANTED`，`TASK_06_STAGE_B_STATUS=IMPLEMENTED_AND_VERIFIED`，`TASK_06_STAGE_C1_STATUS=FINAL_EVIDENCE_READY`，`TASK_06_OWNER_REVIEW=WAITING_AT_OWNER_REVIEW_GATE`，`TASK_06_STAGE_C2_STATUS=NOT_ACCEPTED`，`TASK_06_STAGE_C2_AUTHORIZATION=NOT_GRANTED`，`TASK_07_PLUS_STARTED=NO`。本文件及其他冻结文件中的旧 `PHASE_1B_STARTED=NO`、`PROJECT_OWNER_ACCEPTANCE=PENDING` 仅属 `HISTORICAL/FROZEN` 记录；当前 API 为 Fastify 5.12.1，runtime 为 Node 24.14.0 与 pnpm 11.22.0，T12 为 `T02 -> T03 -> T05`，T10 为 `T05 -> T08 -> T12`。
