...
本仓库用于“学生托管机构智能化系统平台”的多租户 SaaS 规划、原型与早期基础实现。Phase 1A Batch A、Batch B 均为 `HISTORICAL/FROZEN`；当前追认 Phase 1B Task 01—04 已完成，Task 05 Stage B 已完成并已形成 C1 证据，当前停在负责人审阅闸门。

当前活动治理以 `docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V4.md` 为准（SHA-256：`11FD81FB5E9738B36F7EB495424F9D4161F6F5172DFDBA564BE1D5F0FD88DB5E`）。Task 01—05 已由负责人验收，Task 06 尚未启动且未获实现授权。任何新任务仍须独立合同、稳定 SHA、项目负责人批准和单独实现授权；不得从已完成 Task 01—05 自动推导后续任务授权。
...
Batch B 固定交付为 47 个 A 级页面、12 个 B 级流程、59 个浏览器路由、15 张真实浏览器截图、V0.1 技术基线、18 个独立 TDD 任务的 Phase 1B 计划、自动验证器及 86 项精确白名单审查包。上述 Batch B 合同与证据保持历史冻结。当前 Phase 1B 活动治理以 `docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V4.md` 为准（SHA-256：`11FD81FB5E9738B36F7EB495424F9D4161F6F5172DFDBA564BE1D5F0FD88DB5E`）：Task 05 Stage B、Stage C1、Stage C2 均已完成并冻结，`PHASE_1B_STARTED=YES_FOR_TASK_01_TO_TASK_05_ONLY`，`TASK_04_STARTED=YES`，`TASK_04_IMPLEMENTATION_AUTHORIZATION=GRANTED`，`TASK_05_STARTED=YES`，`TASK_05_IMPLEMENTATION_AUTHORIZATION=GRANTED`，`TASK_05_STAGE_C1_STATUS=FINAL_EVIDENCE_READY`，`TASK_05_STAGE_C2_STATUS=ACCEPTED`，`TASK_05_OWNER_REVIEW=ACCEPTED_AND_FROZEN`，`TASK_06_STARTED=NO`，`TASK_06_IMPLEMENTATION_AUTHORIZATION=NOT_GRANTED`。本文件及其他冻结文件中的旧 `PHASE_1B_STARTED=NO`、`PROJECT_OWNER_ACCEPTANCE=PENDING` 仅属 `HISTORICAL/FROZEN` 记录；当前 API 为 Fastify 5.12.1，runtime 为 Node 24.14.0 与 pnpm 11.22.0，T12 为 `T02 -> T03 -> T05`，T10 为 `T05 -> T08 -> T12`。
