...
minimal two-file scope; Task 13 Stage A passed owner review and Stage B is
authorized but not started; Task 13 C1/C2 and Task 14+ remain unauthorized.

Current governance authority:
`docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V28.md`
(`SHA-256=433E07B66ECE98110255158DA63D3FE87D9F4D11FA442444B150FC9FBEFFE4AC`).
The authority uses a whole-file SHA with no self-reference; its final SHA is
recorded after independent Node and .NET verification. Task 13 Stage A passed
owner review, and Stage B is authorized but not started within its exact
four-file boundary; Task 13 C1/C2 and Task 14+ remain unauthorized.
...
`apps/api/src/modules/teachers/public-profile.test.ts`. Under the historical
Task 07 repair and entry-integration authorization, Stage B also includes the
exact expanded boundary recorded in the Task 07 contract and plan: API/admin
...
evidence is frozen and C2 is authorized only for its acceptance record; no
Task 08+ artifact is created by this stage.
...
boundaries; Task 09 is frozen. Task 10 Stage B, C1 evidence, and C2
acceptance are complete and accepted under their separate boundaries; Task 10
is frozen. Task 11 C2 is accepted and frozen. Task 12 new work and Task 13+
are not authorized by the current plan.
...
- Stage B files: `apps/api/src/modules/resources/resource.service.ts`, `apps/api/src/routes/staff-resources.route.ts`, `apps/user-web/src/pages/resources.tsx`, `apps/api/src/modules/resources/resource.test.ts`, `apps/user-web/package.json`, `apps/user-web/tsconfig.json`.
...
- Toolchain constraint: the two user-web files may only use the existing root TypeScript and ESLint toolchain; no new dependency, root package, or lockfile change is allowed.
...
`b856fd809b2ee00e12907d1fd28a5c6186e6b8a2`). Under the active V28 authority,
...
Task 13 Stage B is authorized but not started; Task 13 C1/C2 and Task 14+
remain unauthorized.
