...
为准（SHA-256：`209124CDD9FBB355A9C29610E97439CCBC847A3F5CA033575B14646C7974F833`）。
authority 使用全文件、无自引用 SHA 约定；Task 01—12 已接受并冻结；Task 12
新工作未启动且未授权；Task 13+ 未启动且未授权。
...
此前 V20 按 `CHANGE_CONTROL.md` §5 完成了 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`；该记录保持为历史依据。V23 及更早 authority、Task 01—10 合同、验收、C1 证据和 Batch A/Batch B 固定证据均保持原样；Task 11、Task 12 的 Stage A/C1/C2 文件保持受保护或冻结证据。Task 12 C2 已接受并冻结，但 `apps/api/src/server.ts` 尚未注册 `registerFileIntentRoutes`，route repair 未获授权。本次 V24 将当前活动治理引用统一到 V24，并记录 Task 11 C2 已接受并冻结，停止在 Task 12 新工作之前。
