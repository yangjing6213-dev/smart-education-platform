...
当前 Phase 1B Task 01—09 与后续授权状态的活动治理以
`docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V18.md`
为准（SHA-256：`20157D3F8B0DC997E380E75961BB308E66FCB0BD71214FACA2A713460BC7343E`）。
authority 使用全文件、无自引用 SHA 约定；Task 01—08 已完成并冻结，Task
08 C2 和 Task 09 C2 均为 acceptance-record-only 验收记录；Task 09 原模块证据、
Stage B entry repair、C1 evidence 和 C2 acceptance 已完成并接受/冻结。
...
本次按 `CHANGE_CONTROL.md` §5 采用 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`：原子范围精确为 V18 authority、全部活动治理引用文件和 Task 09 C2 accepted/frozen 状态。V17 及更早 authority、Task 01—08 合同、验收、C1 证据和 Batch A/Batch B 固定证据均保持原样；Task 09 acceptance 保持冻结，Task 10+ 文件不在本轮授权范围内。
