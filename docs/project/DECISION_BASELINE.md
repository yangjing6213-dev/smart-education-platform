...
当前 Phase 1B Task 01—08 与 Task 09 授权状态的活动治理以
`docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V17.md`
为准（SHA-256：`CF8A20D84EE25F4848E2D3CFE43802AF0BEEE96AE17BF4044038E6B695F92917`）。
authority 使用全文件、无自引用 SHA 约定；Task 01—08 已完成并冻结，Task
08 C2 为 acceptance-record-only 验收记录，Task 09 原模块证据和 Stage B
entry repair 已完成并通过负责人审阅，C1 已获授权但尚未开始。
...
本次按 `CHANGE_CONTROL.md` §5 采用 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`：原子范围精确为 V17 authority、全部活动治理引用文件和 Task 09 Stage C1 review/manifest/ZIP 边界。V16 及更早 authority、Task 01—08 合同、验收、C1 证据和 Batch A/Batch B 固定证据均保持原样；Task 09 C2 acceptance 与 Task 10+ 文件不在本轮授权范围内。
