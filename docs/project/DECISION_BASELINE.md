...
为准（SHA-256：`D8008CC04ADBAA95DAF9D002B7FAEEDBB598067E71EB576708C9C67BB03CF4D6`）。
...
此前 V20 按 `CHANGE_CONTROL.md` §5 完成了 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`；该记录保持为历史依据。V24 及更早 authority、Task 01—11 合同、验收、C1 证据和 Batch A/Batch B 固定证据均保持原样；Task 11、Task 12 的 Stage A/C1/C2 文件保持受保护或冻结证据。Task 12 C2 已接受并冻结。本次 V25 将当前活动治理引用统一到 V25，并记录负责人已授权仅针对 `apps/api/src/server.ts` 与 `apps/api/src/modules/files/file.test.ts` 的最小 route repair；不扩大为其他实现或后续任务。
