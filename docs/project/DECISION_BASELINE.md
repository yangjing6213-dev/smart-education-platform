...
为准（SHA-256：`9A6C417329E8A95372DBC39D1EF1F83F1A634E10C456FCA9F9C01AF6D6DA7D19`）。
authority 使用全文件、无自引用 SHA 约定；Task 01—09 已接受并冻结；Task
10 Stage A 已通过负责人审阅但 Stage B 未授权；Task 11 未启动；Task 12
Stage B 已完成并通过负责人审阅，Stage C1 已授权。
...
本次按 `CHANGE_CONTROL.md` §5 采用 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`：原子范围精确为 V19 authority、七个活动入口/基线和 Task 12 合同/计划。V18 及更早 authority、Task 01—09 合同、验收、C1 证据和 Batch A/Batch B 固定证据均保持原样；Task 10 Stage A 文件保持受保护快照，Task 12 C2 与 Task 13+ 不在本轮授权范围内。
