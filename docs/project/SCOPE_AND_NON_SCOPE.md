...
此前 V20 按 `CHANGE_CONTROL.md` §5 完成了 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`，该记录保持为历史依据。V24 及更早 authority、Task 01—11 合同、验收、C1 证据和 Batch A/Batch B 固定证据仍为冻结引用；Task 11、Task 12 的 Stage A/C1/C2 文件保持受保护或冻结证据。Task 12 C2 已接受并冻结。本次 V25 将当前活动治理引用统一到 V25，并记录仅限 `apps/api/src/server.ts` 与 `apps/api/src/modules/files/file.test.ts` 的已授权最小 route repair；Task 13+ 仍未启动且未授权。
