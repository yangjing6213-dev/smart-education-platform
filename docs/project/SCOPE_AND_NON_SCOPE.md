...
当前 Phase 1B Task 01—16 已接受并冻结；Task 12 post-acceptance minimal route repair
已实现、验证、通过负责人审阅并冻结；Task 14 Stage B/C1/C2 与 Task 16 Stage B/C1/C2
已完成并通过负责人审阅且冻结；V41 治理同步已通过负责人审阅并冻结；Task 17 Stage A 已通过负责人审阅，
Stage B 四文件定向技术证据已接受，minimal entrypoint repair 已完成，API/admin-web 入口已接线并通过本地验证，
六类审计动作已完成本地验证；Task 17 C1 已完成、C2 已接受并冻结；`UI_NOT_VISUALLY_VERIFIED=YES` 保留为已知限制；Task 18+ 尚未启动且未获授权。
活动治理以
`docs/project/PHASE_1B_GOVERNANCE_AND_API_FRAMEWORK_AUTHORITY_V42.md`
为准（SHA-256：`7BECA819534A223D84C4D95FD117FC68C4B9CD4CDFC2E5345AF09E9547D188AB`）。
...
此前 V20 按 `CHANGE_CONTROL.md` §5 完成了 `HISTORICAL_FREEZE_PLUS_ACTIVE_REFERENCE_CLOSURE`，该记录保持为历史依据。V25 及更早 authority、Task 01—11 合同、验收、C1 证据和 Batch A/Batch B 固定证据仍为冻结引用；Task 11、Task 12 的 Stage A/C1/C2 文件保持受保护或冻结证据。Task 12 C2 已接受并冻结。本次 V26 将当前活动治理引用统一到 V26，并记录仅限 `apps/api/src/server.ts` 与 `apps/api/src/modules/files/file.test.ts` 的最小 route repair 已实现、验证并通过负责人审阅；Task 13+ 仍未启动且未授权。
